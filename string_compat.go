...
func (i item) String() string {
	...
	case itemSingleEscape:
		escapes := map[string]string{"b": "\b", "f": "\f", "n": "\n", "r": "\r", "t": "\t", "v": "\v"}
		if out, ok := escapes[i.val]; ok {
			return out
		}
		return i.val
...
func newStringLexer(in string) *stringLexer {
	return &stringLexer{str: in, items: make([]item, 0)}
}
...
func (s *stringLexer) Emit(t itemType) {
	s.items = append(s.items, item{typ: t, val: s.str[s.start:s.pos]})
	s.start = s.pos
}
